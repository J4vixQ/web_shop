    ...
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) {  // 用户已经存在
                ...
                const addSqlParams = [phone, phone, 'http://47.120.8.10:' + config.port + '/avatar_uploads/avatar_default.jpg'];  // 手机验证码注册，默认用手机号充当用户名
                conn.query(addSql, addSqlParams, (error, results, fields) => {
                    results = JSON.parse(JSON.stringify(results));
                    if (!error) {
    ...
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) {  // 用户已经存在
                ...
                const addSqlParams = [user_name, user_pwd, 'http://47.120.8.10:' + config.port + '/avatar_uploads/avatar_default.jpg'];
                conn.query(addSql, addSqlParams, (error, results, fields) => {
                    results = JSON.parse(JSON.stringify(results));
                    if (!error) {
                        req.session.userId = results.insertId;
...
router.post('/api/change_user_msg', (req, res) => {
    ...
    form.parse(req, (err, fields, files)=>{
        ...
        let user_avatar = 'http://47.120.8.10:' + config.port + '/avatar_uploads/avatar_default.jpg';
        if(files.user_avatar){
            user_avatar = 'http://47.120.8.10:' + config.port + '/avatar_uploads/' + basename(files.user_avatar.path); 
        }
...
router.post('/api/add_shop_recom', (req, res) => {
    ...
    form.parse(req, (err, fields, files)=>{
		...
		let thumb_url = 'http://47.120.8.10:' + config.port + '/uploads/' + basename(files.goods_img.path);
		let image_url = 'http://47.120.8.10:' + config.port + '/uploads/' + basename(files.goods_img.path);
		let hd_thumb_url = 'http://47.120.8.10:' + config.port + '/uploads/' + basename(files.goods_img.path);
